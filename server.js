...
app.get("/dashboard/:userId", (req,res)=>{
  ...
  db.all("SELECT * FROM habits WHERE user_id=?",[userId],(err,rows)=>{
    ...
    db.all("SELECT * FROM workouts WHERE user_id=?",[userId],(err,rows)=>{
      data.workouts = labels.map(d=>rows.filter(r=>new Date(r.day).toLocaleString("en-US",{weekday:"short"})===d).length);
      data.workouts_history = rows;

      db.all("SELECT * FROM meals WHERE user_id=?",[userId],(err,rows)=>{
        data.meals = labels.map(d=>rows.filter(r=>new Date(r.day).toLocaleString("en-US",{weekday:"short"})===d).length);
